...
function makeProb(condition: Condition, threshold: number): number {
  ...
  if (condition === "healthy") return clamp01(threshold * (0.2 + Math.random() * 0.4));
  if (condition === "warning") return clamp01(threshold * (0.9 + Math.random() * 0.4));
...
export function genereateSimulatedData(
  ...
  const overallMin = Math.min(overallMean, jitter(base.overall.vibration_stats.min, 10));
  const overallMax = Math.max(overallMean, jitter(base.overall.vibration_stats.max, 10));
  ...
  const minProb = clamp01(Math.min(...windows.map((w) => w.faulty_probability)));
  const maxProb = clamp01(Math.max(...windows.map((w) => w.faulty_probability)));
